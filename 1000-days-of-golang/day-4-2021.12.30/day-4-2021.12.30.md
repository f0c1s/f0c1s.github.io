...
This is a public function that I was using earlier, but in the last example, I am doing exactly what this function is
doing.
...
The `net.Listen` is `func Listen(network, address string) (Listener, error)` and thus "tcp" is `network` and ":http"
is `address`.
...
`Addr` is string that can have "host:port" or ":port" or "". Missing values for host and port are filled as "http" and "
80" respectively.

`Handler` is function that can handle incoming requests. `http.DefaultServeMux` is the default value, in most cases
handler is `nil`.

`TLSConfig` is used for HTTPS servers. I guess.

`ReadTimeout` is max time a request can take reaching the server. A positive value can protect
from [slow-loris](https://en.wikipedia.org/wiki/Slowloris_(computer_security)){: .in} attack.

`ReadHeaderTimeout` is max time all the request headers can take reaching the server. A positive value can protect from
slow-loris.

Of `ReadHeaderTimeout` is zero, `ReadTimeout` is read.

`WriteTimeout` is time out for writing a response. I do not understand what they mean by "It is reset whenever a new
request's header is read.".

`IdleTimeout` is max time to wait for the next request when keep-alives are enabled. We will use it certainly for our
server soon.
...
I can get rid of the message "server will not serve: http: Server closed" by not caring if server was started or not,
but meh.
